...
const PCApp = {
  ...
  renderRightCol() {
    ...
    // 2. Real-time Notifications Widget (메인 대시보드 실시간 상황변화 알림 위젯)
    const notiWrap = document.getElementById('pc-widget-notifications');
    if (notiWrap) {
      const visibleNotifs = typeof this.getVisibleNotifications === 'function'
        ? this.getVisibleNotifications()
        : (this.state.notifications || []);
      const topNotifs = visibleNotifs.slice(0, 3); // 상위 3개 표시
      const unreadCount = typeof this.getUnreadNotificationCount === 'function' ? this.getUnreadNotificationCount() : 0;
        ...
        <div class="pc-bento-card">
          <div class="pc-card-header">
            <span class="pc-card-title">
              ...
              실시간 알림
              ${unreadCount > 0 ? `<span class="px-2 py-0.5 rounded-full text-xs font-bold bg-[#e83538] text-white ml-1">${unreadCount}</span>` : ''}
            </span>
            <button class="pc-card-action" onclick="PCApp.openNotificationModal()">전체보기</button>
          </div>

          <div class="space-y-2.5">
            ${topNotifs.length > 0 ? topNotifs.map(item => {
              let typeBadge = '';
              if (item.type === 'commute') {
              ...
              <div class="py-8 flex flex-col items-center justify-center text-center text-on-surface-variant">
                <svg class="w-8 h-8 text-outline mb-2" viewBox="0 -960 960 960" fill="currentColor">
                  <path d="M160-200v-80h80v-280q0-83 50-147.5T420-792v-28q0-25 17.5-42.5T480-880q25 0 42.5 17.5T540-820v28q80 20 130 84.5T720-560v280h80v80H160Zm320-300Zm0 420q-33 0-56.5-23.5T400-160h160q0 33-23.5 56.5T480-80ZM320-280h320v-280q0-66-47-113t-113-47q-66 0-113 47t-47 113v280Z"/>
                </svg>
                <p class="text-xs font-bold text-on-surface">도착한 알림이 없습니다.</p>
              </div>
            `}
          </div>
        </div>
