...
- **할 일 관리(`screen-todo`) 계층적 프로젝트 목록 & 클릭 시 진행도(대기/진행 중/완료) 수평 칸반 보드 전환 구조 개편**:
  - 할 일 관리 첫 화면에서 등록된 프로젝트별 카드(프로젝트명, 총 업무 수, 대기/진행 중/완료 진행도 칩, 최근 업무 미리보기)를 그룹형으로 먼저 표출.
  - 특정 프로젝트 카드를 클릭 시 해당 프로젝트 내부의 '대기 / 진행 중 / 완료' 3단계 진행도별 수평 스크롤 칸반 보드(`kanban-column`)로 전환되며, 뒤로가기 버튼으로 언제든지 전체 프로젝트 목록으로 복귀 가능.
  - 칸반 보드 내 카드 클릭 시 기존의 읽기 전용 상세 모달(`modal-todo-detail`) 팝업 연동 유지.
