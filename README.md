...
- **할 일 관리(`screen-todo`) 한 줄 간략 뷰 모드(`todoViewMode === list`) 토글 복원 & 렌더링 보강**:
  - 우측 상단 뷰 토글 버튼 클릭 시 동작하는 '한 줄 간략 모드' 렌더링 분기를 복원하여, 전체 및 프로젝트별 할 일 항목들이 완료 체크박스, 우선순위 도트, 프로젝트 태그, 마감일 및 간결한 한 줄 리스트로 즉시 전환되어 조회될 수 있도록 수정.
