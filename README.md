...
- **2026년 8월 19일 이후 캘린더 근태/외근/휴가 일정 데이터 최신화**:
  ...
  - 8월 31일: 최지영(연차).
- **주소록 임직원 애니메이션(지브리) 프로필 사진 전체 복원**:
  - 임직원 주소록용으로 제작된 전사 지브리 애니메이션 프로필 이미지 파일(`profile_ghibli_*.jpg`) 및 데이터베이스 매핑(`employees.json`, `mockData.js`, `firebase-seed.json`) 원상 복원 완료.
