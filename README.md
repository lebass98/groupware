...
- **할 일 작성/수정 팝업(`modal-todo-write`) 진행 상태 선택 & 상태 직접 입력 기능 신설**:
  - 할 일 신규 작성 및 수정 모달 내에 **'진행 상태(대기 / 진행 중 / 완료)'** 선택 버튼 폼 추가.
  - 기본 상태 이외에도 **'새로운 상태 직접 입력(예: 검토 중, 보류)'** 영역을 탑재하여 사용자가 직접 정의한 맞춤형 진행 상태를 즉시 설정 및 등록할 수 있도록 보강.
