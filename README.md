...
- **할 일 관리 프로젝트 칸반 보드 웹 브라우저 마우스 클릭 앤 드래그 수평 스크롤(Mouse Drag-to-Scroll) 구현**:
  - 모바일 터치 스와이프뿐만 아니라 PC/웹 브라우저 마우스 클릭 앤 드래그 인터랙션(`initKanbanDragScroll`) 및 `cursor: grab / grabbing` 스타일을 추가하여, 데스크톱 환경에서도 마우스로 대기/진행 중/완료 컬럼을 손쉽게 좌우로 쓱 잡아 끌어 스크롤할 수 있도록 보강.
