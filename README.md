...
- **할 일 관리 프로젝트 내부 뷰 대기 / 진행 중 / 완료 세로 배치 & 행별 카드 가로(수평) 스크롤 구조 개편**:
  - 프로젝트 내부 뷰 진입 시 **'대기', '진행 중', '완료' 3개 진행도 구분을 위에서 아래로 세로(수직) 배치**.
  - 각 구획(대기/진행 중/완료) 내부의 할 일 카드들은 **가로(수평) 스크롤/드래그(`kanban-row-scroll-container`)**되도록 구성하여, 수직 탐색과 수평 카드 확인이 직관적으로 이루어지는 모바일/웹 겸용 UX 완비.
