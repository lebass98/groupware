...
- **2026년 8월 19일 이후 캘린더 근태/외근/휴가 일정 데이터 최신화**:
  ...
  - 8월 31일: 최지영(연차).
- **주소록 임직원 프로필 사진 중복 참조 수정 및 고유 원본 아바타 복원**:
  - 이전 지브리 스타일 변환 과정에서 일부 이미지가 전사 여러 임직원에게 중복 복사/매핑되어 사진이 겹쳐 보이던 문제를 확인하고 전면 수정.
  - 전사 21명 모든 임직원의 100% 고유한 원본 프로필 사진(`profile_*.png`)을 데이터베이스(`employees.json`, `mockData.js`, `firebase-seed.json`)에 정상 재매핑하고 중복 파일 정리 완료.
