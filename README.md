...
- **구글 파이어베이스(Firebase) 연동을 위한 데이터 추출 및 모듈 구조화**:
  - 그룹웨어 애플리케이션의 모든 목업 데이터(임직원 주소록, 공지사항, 할 일/휴지통, 일정, 공휴일/절기/기념일, 근태/출퇴근, 경비/결재)를 Firestore DB 컬렉션 구조에 맞게 독립 JSON 데이터 파일(`data/*.json`)로 추출 및 구조화.
