...
- **할 일 관리 한 줄 간략 뷰 모드(`todoViewMode === list`) 프로젝트 계층 구조 완벽 연동**:
  - 메인 화면에서 한 줄 간략 모드를 켰을 때 소프로젝트/할 일 항목이 바로 뜨지 않고, **카드 모드와 동일하게 프로젝트 단위 한 줄 리스트**로 먼저 표출되도록 수정.
  - 한 줄 리스트 항목 클릭 시 **해당 프로젝트 내부(`selectedProject`)로 정상 진입**하며, 프로젝트 내부에서 소속된 소프로젝트/할 일 항목들이 한 줄로 노출되도록 계층적 뷰 흐름 개편.
