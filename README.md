...
- **할 일 관리(`screen-todo`) 수평 스크롤 칸반 보드(대기 / 진행 중 / 완료) 시스템 구현**:
  - 기존 단일 리스트 뷰를 모바일 환경에 최적화된 수평 스크롤/드래그 칸반 보드로 전면 개편.
  - '대기', '진행 중', '완료' 3단계 진행도별 가로 정렬 컬럼(`kanban-column`)을 구성하고, 각 컬럼 내부에서는 할 일 카드들을 세로로 정렬.
  - 칸반 보드 내 카드 클릭 시 기존의 읽기 전용 상세 모달(`modal-todo-detail`) 오픈 연동 및 검색/필터/뷰 모드 토글 지원.
