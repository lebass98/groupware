...
- **휴가 신청 폼 내 9종 휴가 종류 및 '반반차' 시간 선택 폼 연동**:
  - 휴가 종류 옵션을 9종(연차, 반차(오전), 반차(오후), 반반차, 공가, 병가, 경조사, 생일휴가, 기타)으로 전면 확대 적용.
  - **'반반차'** 선택 시 시작시간 및 종료시간을 직접 선택할 수 있는 2개 입력 박스 구획이 동적으로 표시되며, '총 0.25일' 및 선택된 시간대 정보가 자동 계산/표시되도록 개선.

### 2026-08-14
- **대체공휴일 캘린더 자동 등록 및 조정**:
  - 2026년 3월 2일 (3·1절 대체공휴일), 5월 25일 (부처님오신날 대체공휴일), 8월 17일 (광복절 대체공휴일), 10월 5일 (개천절 대체공휴일) 등 대체공휴일 데이터를 캘린더 및 일정 시스템에 등록하고 캘린더 그리드 상 빨간 날(공휴일) 색상 표시 적용.
  - 9월 28일 대체공휴일 데이터 삭제 처리.
- **구글 파이어베이스(Firebase) 연동을 위한 데이터 추출 및 모듈 구조화 (100% 데이터 완전 분리 완료)**:
  - 그룹웨어 애플리케이션의 모든 비즈니스 데이터(임직원 주소록, 공지사항, 할 일/휴지통, 일정, 공휴일/절기/기념일, 근태/출퇴근, 경비/결재)를 Firestore DB 컬렉션 구조에 맞게 독립 JSON 파일(`data/*.json`) 및 JS 모듈(`data/mockData.js`)로 추출 및 완전 분리.
  - `script.js` 내부의 3,500여 줄에 달하던 하드코딩 데이터 객체를 전면 제거하고 `window.MockData` 외부 데이터 모듈 참조 방식으로 개편하여 애플리케이션 코어 로직과 데이터 레이어를 100% 독립 분리.
...
- **광복절 중복 데이터 1개만 유지 보정**:
  - 8월 15일 광복절 항목 중 데이터 맵(`schedules`)에 중복으로 등록되어 있던 1개 항목을 제거하여 광복절이 단 1개만 정상 표시되도록 처리 (자동 중복 제거 로직은 요청에 따라 취소 및 기존 비교 유지).
...
- **전사 임직원 프로필 이미지 지브리 애니메이션 스타일 전체 변환 적용**:
  - 퍼블리싱팀 및 디자인팀에 이어 경영지원팀(김경현 대표, 오은주 차장), 기획팀(김종규 팀장, 박규태 대리, 한상희 사원, 장현아 수습), 개발팀(최우석 과장, 안영재 대리, 곽재훈 대리, 유종현 주임), 전략본부(남기현 본부장, 윤진성 과장), 수행본부(김종규 본부장, 이채원 사원) 등 그룹웨어 주소록 내 **전사 21명 모든 임직원의 프로필 아바타 사진**을 스튜디오 지브리 애니메이션 아트 스타일로 일괄 생성/매핑 적용.
  - `mockData.js`, `employees.json`, `firebase-seed.json` 데이터베이스 및 기존 프로필 리소스 파이프라인 일체 업데이트 완료.
...
### 2026-08-20
- **2026년 8월 19일 이후 캘린더 근태/외근/휴가 일정 데이터 최신화**:
  - 8월 19일부터 8월 31일까지의 전사 임직원 연차, 반차, 반반차, 외근 일정 데이터 업데이트 (`data/schedules.json`, `data/mockData.js`, `data/firebase-seed.json`).
  - 8월 19일: 이재광(연차), 한상희(연차), 손석호(외근-한가원 주간보고), 이채원(외근-한국건강가정진흥원 주간회의), 장현아(외근-한국건강가정진흥원 주간회의), 윤진성(외근-한가원 주간보고).
  - 8월 20일: 오은주(오후 반차), 남기현(외근-에스엠티 미팅).
  - 8월 21일: 박규태(오후 반차), 남기현(외근-시티시큐 방문), 윤익수(반반차 16:00~18:00) (이재광 반차 삭제 반영).
  - 8월 24일: 남기현(오전 반차).
  - 8월 25일: 남기현(외근-용인문화원 방문), 이채원(외근-용인디지털기록관 회의).
  - 8월 31일: 최지영(연차).
- **주소록 임직원 프로필 사진 실사(원본) 사진으로 교체**:
  - 임직원 주소록 프로필 아바타를 각 임직원 고유의 실물 실사 사진(`profile_*.png`)으로 데이터베이스(`employees.json`, `mockData.js`, `firebase-seed.json`) 매핑 교체 완료. (기존 지브리 애니메이션 이미지 파일 `profile_ghibli_*.jpg`은 보존)
- **주소록 임직원 기본 근무 상태 및 오늘 예정 일정(오후 반차/외근 등) 분리 병기 UI 구현**:
  - 오은주 차장 등 오늘 반차/외근 예정이 있는 직원의 경우 현재 실시간 상태는 기본 `[🟢 근무중]`으로 명확히 표시하고, 옆/상세에 `[📅 오늘 예정: 오후 반차 (13:00~18:00)]` 등의 예정 일정 뱃지를 함께 표시하도록 로직 개선.
  - 전사 임직원 데이터베이스(`employees.json`, `mockData.js`, `firebase-seed.json`) 및 캘린더 연동을 통해 기본 상태(근무중/외근중/휴가중/퇴근)와 당일 예정 스케줄을 직관적으로 시각화.
