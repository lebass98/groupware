...
const App = {
  state: {
    isLoggedIn: false, // Default to FALSE so Login screen is always shown first
  ...
  init() {
    ...
    // Always start at Login screen on load
  ...
  // Called when user clicks Pulse Button (Open Confirm Dialog)
  toggleCheckIn() {
  ...
  loginDemo(provider) {
    ...
    const msg = provider 
      ...
      : `🎉 ${this.state.user.name}님, 데모 로그인 완료! 출결 관리 화면으로 이동합니다.`;
    this.showToast(msg);
  },
  ...
  logout() {
    ...
    this.hideAppShell();
    this.showScreen('screen-login');
    this.showToast('로그아웃 되었습니다. 다시 이용하시려면 로그인해주세요.');
  },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Dark mode header toggle button listener
  const themeBtn = document.getElementById('theme-toggle-btn');
  if (themeBtn) {
    themeBtn.addEventListener('click', () => {
      App.toggleDarkMode(!App.state.settings.dark);
    });
  }
});
